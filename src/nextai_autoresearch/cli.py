...
from .laboratory import laboratory_contract, laboratory_progress
from .provenance import candidate_provenance
    ...
    if not state_path.exists():
        ...
        atomic_write_json(
            state_path,
            {
                ...
                "generation": int(config.raw["project"]["generation"]),
...
def command_lab_status(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({
        ...
        "scoring_ready": report.ok and contract["scoring_authorized"] and load_config().benchmark_status == "active" and not progress["user_decision_required"],
        "status": contract["status"],
        **progress,
        "plan": "research/LAB_PLAN.md",
        "errors": report.errors,
        "warnings": report.warnings,
        "schedule_checked_or_started": False,
    }, indent=2, ensure_ascii=False))
    return 0 if report.ok else 1


def command_provenance(args: argparse.Namespace) -> int:
    result = candidate_provenance(args.experiment, args.candidate, revision=args.revision)
    print(json.dumps(result, indent=2, ensure_ascii=False))
    return 0 if result["ok"] else 1
...
def command_plan_new(args: argparse.Namespace) -> int:
    if getattr(args, "pc01_phase", None):
        ...
        if len(args.candidates) != 1:
            raise ValueError("PC-01 takes one source-identical model factory, not candidate roles")
        print(create_plan(project_root(), candidate=args.candidates[0], phase=args.pc01_phase,
                          question=args.question, series_path=args.pc01_series))
        return 0
    legacy_required = ("hypothesis", "title", "family", "prediction", "kill_criterion", "promotion_criterion",
                       "alternative", "confound", "positive_conclusion", "null_conclusion", "negative_conclusion")
    missing = [name for name in legacy_required if not getattr(args, name, None)]
    if missing:
        raise ValueError(f"Legacy plan requires: {', '.join(missing)}")
    root = project_root()
    ensure_layout(root)
    ...
    if config.benchmark_version in {
        "latent_entity_binding_retrieval_v2",
        "latent_entity_binding_retrieval_v3",
    }:
        addressing = config.raw[
            "entity_addressing" if config.benchmark_version.endswith("_v2")
            else "hierarchical_addressing"
        ]
        ...
        protocol_keys = (
            ...
            ) if config.benchmark_version.endswith("_v2") else (
                "observation_dimension", "encoder_widths", "fit_epochs", "batch_size",
                "optimizer", "learning_rate", "tree_depth_rule", "split_objective",
                "split_ties", "beam_width", "visited_node_cap", "verifier",
        ...
        plan["entity_addressing_protocol"] = {
            key: addressing[key] for key in protocol_keys
        }
    if config.benchmark_version in {
    ...
    if config.benchmark_version.startswith("heldout_parallel_masked_"):
        ...
        is_stack = config.benchmark_version.endswith(
            ("_v8", "_v9", "_v10", "_v11", "_v12", "_v13")
        )
        ...
        if config.benchmark_version in {
            ...
            "heldout_parallel_masked_infilling_v12",
            "heldout_parallel_masked_infilling_v13",
        }:
            plan["matrix"]["knowledge_sizes"] = list(active["knowledge_sizes"])
        ...
        if config.benchmark_version in {
            ...
            if is_stack:
                ...
                protocol.update({
                    ...
                    "task_unit": (
                        "balanced_real_python_recoverable_push_chain"
                        if config.benchmark_version.endswith("_v13")
                        else "balanced_real_python_closure_chain"
                        if config.benchmark_version.endswith(
                            ("_v9", "_v10", "_v11", "_v12")
                        )
...
def command_pc01_freeze_series(args: argparse.Namespace) -> int:
    from .pc01_execution import freeze_series
    print(freeze_series(project_root(), args.selected_dev))
    return 0


def command_pc01_series_status(args: argparse.Namespace) -> int:
    from .pc01_execution import authenticated_series_decision
    print(json.dumps(authenticated_series_decision(project_root()), indent=2))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pc01_series.set_defaults(func=command_pc01_freeze_series)
    pc01_status = lab_sub.add_parser("pc01-series-status", help="Verify all registered final attempts; never promotes an architecture")
    pc01_status.set_defaults(func=command_pc01_series_status)

    provenance = subcommands.add_parser("provenance", help="Resolve historical audited source hashes without executing code")
    provenance.add_argument("--experiment", required=True)
    provenance.add_argument("--candidate", required=True)
    provenance.add_argument("--revision", help="Explicit historical Git commit hash")
    ...
    new.add_argument("--pc01-phase", choices=["dev", "final"])
    new.add_argument("--pc01-series", type=Path)
    ...
    new.add_argument("--kill-criterion", action="append")
    new.add_argument("--promotion-criterion", action="append")
    new.add_argument("--alternative", action="append")
    new.add_argument("--confound", action="append")
    new.add_argument("--positive-conclusion")
    new.add_argument("--null-conclusion")
    new.add_argument("--negative-conclusion")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (FileNotFoundError, FileExistsError, RuntimeError, ValueError, ValidationError) as exc:

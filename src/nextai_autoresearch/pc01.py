...
METADATA_COHORT = "pc01_byte_lm_learning_measurement_v3"
COHORTS = (COHORT, TELEMETRY_COHORT, METADATA_COHORT)
GIB = 1024**3
CONTROL_NAMES = (
...
def series_decision(records: list[dict], *, root: Path | None = None, cohort: str = COHORT) -> dict:
    ...
    try:
        require(cohort in COHORTS, "unknown series cohort")
        ...
        for record in records:
            if cohort == METADATA_COHORT:
                import copy
                from jsonschema import Draft202012Validator
                from .schemas import load_schema
                from .pc01_gpu_metadata import validate_pair
                schema = copy.deepcopy(load_schema("pc01_replica", root))

...
from nextai_autoresearch.cross_family_contract import (
    PrivilegedQuery, PrivilegedTraining, PrivilegedUpdate,
)


@dataclass(frozen=True)
class _OracleProgram:
    program: tuple[int, ...]
...
class SpecialistSuite(CandidateBase):
    metadata = CandidateMetadata(
        "cross-family-specialist-suite", "specialist_control",
        "Fully charged separate native model for every selected family.",
    )
    ...
    def _model(self, family: str):
        ...
        if family == "probabilistic":
            cls = OracleProbabilistic if oracle else {
                "contextual": Contextual, "joint": Joint, "autoregressive": Autoregressive,
    ...
    def fit(self, facts: PrivilegedTraining, universe_size: int, max_depth: int) -> None:
        if not isinstance(facts, PrivilegedTraining):
            raise TypeError("specialist suite requires its declared privileged envelope")
    ...
    def query(self, source: PrivilegedQuery, steps: int):
        key = (source.public.slot, source.public.signature)
        if key in self.memo:
    ...
    def update(self, source: PrivilegedUpdate, target: object) -> None:
        del target
        key = (source.public.query.slot, source.public.query.signature)
        self.memo[key] = tuple(source.public.target)
        self.update_ops += float(len(source.public.target) + 1)

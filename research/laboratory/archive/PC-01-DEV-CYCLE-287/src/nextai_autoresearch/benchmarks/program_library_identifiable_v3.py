...
from .program_library_adversarial_v2 import make_training_corpus
from .program_library_v1 import Task, load_candidate
from .successor_graph_v1 import percentile


BENCHMARK_VERSION = "program_library_identifiable_v3"
...
def make_task(depth: int, seed: int, index: int) -> Task:
    if depth == 1:
        ...
        raise ValueError("program_library_identifiable_v3 supports depth 1 or depth >= 4")

    inputs = list(range(DOMAIN_SIZE))
    random.Random(seed ^ (index * 104729) ^ (depth << 16)).shuffle(inputs)
    examples = tuple((value, execute(program, value)) for value in inputs[:-1])
    test_input = inputs[-1]
    return Task(program, examples, test_input, execute(program, test_input))

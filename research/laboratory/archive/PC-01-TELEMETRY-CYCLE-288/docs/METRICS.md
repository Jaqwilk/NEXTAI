...
## Protocol-v3 clarification

Learning, economics and transfer are separate claims. A cohort must freeze its
own useful-quality threshold and directions; 0.95 accuracy is not a universal
guard for loss tasks. Historical cohort metrics and thresholds remain unchanged.
Batch=1 end-to-end latency and batched throughput are separate measurements.
Record hardware/software, precision, threads, warm-up, synchronization, all raw
repeats and p50/p95, as well as fit, acquisition, update, RAM/VRAM and bytes moved.
Amortized batch time is not interactive latency; ops are not measured energy.
R must name its workload unit before estimating break-even reuse. A ranking
inversion across hardware/scenarios is not an algorithmic-complexity result.

...
from .pc01 import (COHORT, CONTRACT_SHA256, DATA_SHA256, GIB, contract, require,
                   series_decision)
...
def create_plan(root: Path, *, candidate: str, phase: str, question: str,
    ...
    root = root.resolve()
    ensure_can_create_plan(root)  # No maintenance exception for diagnostics.
    require(load_config(root).benchmark_version == COHORT, "activate the separately frozen PC-01 cohort first")
...
def freeze_series(root: Path, selected_dev_id: str) -> Path:
    ...
    with RunLock(root, stale_seconds=2400):
        ...
        selected = next(p for p in plans if p["experiment_id"] == selected_dev_id)
    ...
    with RunLock(root, stale_seconds=2400):
        ...
        try:
            ...
            result = {"schema_version": 1, "kind": "pc01_diagnostic_result", "experiment_id": identifier,
                      "benchmark": COHORT, "plan_path": plan_path.relative_to(root).as_posix(), "plan_sha256": sha256_json(plan),
...
def authenticated_series_decision(root: Path) -> dict:
    ...
    decision = series_decision(records, root=root)

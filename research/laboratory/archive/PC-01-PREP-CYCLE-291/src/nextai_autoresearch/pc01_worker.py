...
def run(runtime_path: Path, root: Path) -> None:
    ...
    gpu_metadata = None
    if plan["benchmark"] == pc01.METADATA_COHORT:
        from .pc01_gpu_metadata import capture_required
        gpu_metadata = {"before_fit": capture_required(work, "before_fit", root)}
    import torch
    ...
    if gpu_metadata is not None:
        pc01.require(torch.cuda.get_device_name() == gpu_metadata["before_fit"]["gpu"]["name"],
                     "CUDA device differs from metadata GPU")
    ...
    if gpu_metadata is not None:
        ...
        gpu_metadata["after_timing"] = capture_required(work, "after_timing", root)
        validate_pair(gpu_metadata)
    ...
    driver = None
    if gpu_metadata is not None:
        measurement["gpu_metadata"] = gpu_metadata
        driver = gpu_metadata["after_timing"]
    executable = shutil.which("nvidia-smi") if gpu_metadata is None else None
    if executable:

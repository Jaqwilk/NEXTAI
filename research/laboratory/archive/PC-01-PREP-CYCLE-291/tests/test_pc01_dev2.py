...
def scope(tmp_path, monkeypatch):
    ...
    config.write_text(config.read_text().replace(pc01.METADATA_COHORT, pc01.TELEMETRY_COHORT))
...
def test_schema_dispatch_preserves_v1_and_rejects_unknown_version(lab):
    ...
    with pytest.raises(ValidationError):
        validate_document("experiment_plan", {**original, "benchmark": "pc01_byte_lm_learning_measurement_v4"}, lab)
    with pytest.raises(ValidationError):

...
def validate_document(name: str, value: Any, root: Path | None = None) -> None:
    ...
    # V1 bytes/contracts remain immutable. V2 changes only measurement identity;
    # all structural, metric and budget requirements are inherited unchanged.
    if name in ("pc01_plan", "pc01_result") and isinstance(value, dict):
        if value.get("benchmark") == "pc01_byte_lm_learning_measurement_v2":
            schema["properties"]["benchmark"] = {"const": "pc01_byte_lm_learning_measurement_v2"}

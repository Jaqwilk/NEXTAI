...
def test_actual_plan_and_run_gates_block_without_mutation(tmp_path):
    _lab_fixture(tmp_path)
    root = tmp_path
...
def test_lab_status_reports_preparation_not_scoring(monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, "laboratory_contract", lambda: load_json(project_root() / CONTRACT_PATH))
